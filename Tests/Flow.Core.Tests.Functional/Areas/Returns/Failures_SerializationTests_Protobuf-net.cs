...
public class Failure__SerializationTests_Protobuf
{
    ...
    public void Should_be_able_to_serialize_and_deserialize_an_invalid_entry_failure_using_protobuf_net_serializer(string? reason, string? details, int subTypeID, bool canRetry, string? exceptionMessage, string? occurredAtString)
    {
        ...
        var deserializedFailure = Serializer.DeepClone<Failure.InvalidEntryFailure>(failureToSerialize);

        using (new AssertionScope())
        {
            deserializedFailure.Details.Should().BeEquivalentTo(failureDetails ?? []);
