...
public class Flow_ExtensionTests_OnFailure
{
    ...
    [Fact]
    public async Task A_failed_flow_task_should_invoke_the_on_failure_task_func_and_return_its_flow()
    { 
        var successFlow = await Task.FromResult(Flow<int>.Failed(new Failure.ApplicationFailure("Some failure")))
                                        .OnFailure(failure => Task.FromResult(Flow<int>.Success(42)));

        successFlow.Match(failure => throw new XunitException("Should not be a failure as we failed and then ran another method which succeeded"),
                           success => success.Should().Be(42));
    }

    [Fact]
    public async Task A_successful_flow_task_should_not_invoke_the_On_failure_task_func_and_should_only_return_the_current_flow_instance()
    {
        var successFlow = await Task.FromResult(Flow<int>.Success(42))
                                        .OnFailure(failure => Task.FromResult(Flow<int>.Success(84)));
    ...
    [Fact]
    public async Task A_failed_flow_should_invoke_the_on_failure_task_func_and_return_its_flow()
    {
        var successFlow = await Flow<int>.Failed(new Failure.ApplicationFailure("Some failure"))
                                        .OnFailure(failure => Task.FromResult(Flow<int>.Success(42)));
    ...
    [Fact]
    public async Task A_successful_flow_should_not_invoke_the_On_failure_task_func_and_should_only_return_the_current_flow_instance()
    {
        var successFlow = await Flow<int>.Success(42)
                                    .OnFailure(failure => Task.FromResult(Flow<int>.Success(84)));
